...
export class UserComponent implements AfterViewInit, OnInit {
  ...
  deleteUser(id: number) {
    this.userService.deleteUser(id).subscribe(data => {
      ...
      if (index !== -1) {
        this.userList.splice(index, 1); // Kullanıcıyı diziden kaldır
        this.dataSource = new MatTableDataSource(this.userList); // Veri kaynağını güncelle
        this.configDataTable(); // Veri tablosunu yeniden yapılandırma
      }
      this.getUserList();
    });
  }
